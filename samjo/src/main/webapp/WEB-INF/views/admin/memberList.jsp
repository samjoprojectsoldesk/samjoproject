		...
		<div class="container">
			<div class="slider-text-inner desc">
				<h2
					...
					class="heading-section">회원 목록</h2>
			</div>
		</div>
	</div>
					...
					<table class="table table-hover">
						...
						<tbody>
							<c:forEach var="dto" items="${list}">
								<tr>
									<th scope="row">${dto.user_idx}</th>
									<td onclick="location.href='/member/read.do?user_idx=${dto.user_idx}'">${dto.user_id}</td>
									<td>${dto.user_name}</td>
									<td>${dto.user_date}</td>
									<td><button type="button" class="btn btn-danger"
											onclick="location.href='/admin/userdelete.do?user_id=${dto.user_id}'">삭제</button></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
