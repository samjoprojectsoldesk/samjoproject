...
import kr.co.samjo.product.sookso.SooksoDTO;
>>>>>>> a21e6d900fbcea6b228f9bd80a7bef6c187efa54
...
>>>>>>> a21e6d900fbcea6b228f9bd80a7bef6c187efa54
	...
	/* 패키지여행 */
	public ArrayList<packagetourDTO> list3(int start, int end, String word) {
		ArrayList<packagetourDTO> list = null;
		try {
			con = dbopen.getConnection();
			sql = new StringBuilder();
			...
			if (word.length() != 0) {
				String search = "";
				search += " AND pack_name LIKE '%" + word + "%' ";
				sql.append(search);
			}
			...
			System.out.println("패키지 투어 페이징 목록 실패: " + e);
		} finally {
			DBClose.close(con, pstmt, rs);
		} // end
		return list;
	...
	public int totalpackageRowCount() {
		int cnt = 0;
		try {
			con = dbopen.getConnection();
			sql = new StringBuilder();
...
/* 렌트카업체 */
	public ArrayList<rentalDTO> list4(int start, int end, String word) {
		...
		try {
			...
			sql.append("        FROM ( ");
			sql.append(
					"               SELECT u_code, u_name, u_phone, u_office, u_img, u_cont ");
			sql.append("               FROM tb_upche ");

			if (word.length() != 0) {
				String search = "";
				search += " AND u_name LIKE '%" + word + "%' ";
				sql.append(search);
			}
...
/* 렌트카 */
	public ArrayList<rentalcarDTO> list5(int start, int end, String word) {
